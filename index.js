...
function processDuplicateFree(list, callback) {
  ...
  const deDupedList = list.filter(function(item, index){
...
function lowerCaseStrings(strings) {
  ...
  strings.forEach(function(item) {
    ...
    lowerCaseArray.push(item.toLowerCase());
...
function stringSmash(strings) {
  ...
  return strings.reduce(function(accumulator, currString){

    return accumulator + currString;
...
function tallyUpDonations(runners) {
  ...
  return runners.reduce(function(total, current){
 ...
 * 
 * counter1 is using closure to allow the function being returned to increment count without having count reset once the function completes. The value for count is maintained outside of the counterMaker() function scope because of closure.
 * 
 * counter2 is using a global variable for count which can be altered within counter2, while maintaining its value outside the function.
 * 
 * 
 * 2. Which of the two uses a closure? How can you tell?
 * 
 * counter1 uses closure. You can tell from the nested functions.
 * 
 * 
 * 3. In what scenario would the counter1 code be preferable? In what scenario would counter2 be better? 
 * 
 * counter1 is better on larger scale projects where you want little to no global variables that could cause naming conflicts or accidental overrides.
 * 
 * counter2 is better on smaller scale projects where global variables are not an issue and naming conflicts / accidental overrides are very uncommon. counter2 is more efficient / easier to read.
 *
*/
